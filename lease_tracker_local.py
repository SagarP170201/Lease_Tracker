...
CONN_NAME = os.getenv("SNOWFLAKE_CONNECTION_NAME") or "SFSEAPAC-SPAWAR_AWSEAST1"
...
@st.cache_resource
def get_connection():
    ctx = snowflake.connector.connect(connection_name=CONN_NAME)
...
def approve_record(staging_id, file_name, extract):
    ...
    run_dml("""
        INSERT INTO SI_EVENTS_HOL.LEASE_INTELLIGENCE.LEASE_TRACKER (
            FILE_NAME, STORE_NAME, MALL_NAME, CITY, STATE, REGION,
            CARPET_AREA_SQFT, LESSOR_NAME, LESSEE_NAME, BRAND,
            LEASE_START_DATE, LEASE_END_DATE, LEASE_TENURE_MONTHS,
            LOCK_IN_PERIOD_MONTHS, LOCK_IN_EXPIRY_DATE, NOTICE_PERIOD_DAYS,
            RENT_FREE_PERIOD_MONTHS, RENT_MODEL, FIXED_RENT_MONTHLY,
            REVENUE_SHARE_PCT, MINIMUM_GUARANTEE_MONTHLY, RATE_PER_SQFT,
            ESCALATION_PCT, ESCALATION_FREQUENCY_MONTHS, NEXT_ESCALATION_DATE,
            CAM_MONTHLY, HVAC_MONTHLY, MARKETING_CONTRIBUTION_MONTHLY,
            SECURITY_DEPOSIT, STAMP_DUTY, REGISTRATION_COST, CAPEX_REIMBURSEMENT,
            PAYMENT_DUE_DAY, RENEWAL_OPTION, RENEWAL_NOTICE_DAYS, RENEWAL_DATE,
            TOTAL_MONTHLY_OUTGO, STATUS, APPROVED_BY, APPROVED_AT
        ) VALUES (
            %s, %s, %s, %s, %s, %s,
            %s, %s, %s, %s,
            %s, %s, %s,
    ...
    """, (
        file_name,
        extract.get("store_name") or "",
        extract.get("mall_name") or "",
        city,
        extract.get("state") or "",
        CITY_REGION.get(city, "Other"),
        _safe_num(extract.get("carpet_area_sqft")),
        extract.get("lessor_name") or "",
        extract.get("lessee_name") or "",
        extract.get("brand") or "",
        lease_start, lease_end,
        _safe_int(extract.get("lease_tenure_months")),
        lock_in_months, _add_months(lease_start, lock_in_months),
        _safe_int(extract.get("notice_period_days")),
        _safe_int(extract.get("rent_free_period_months")),
        extract.get("rent_model") or "FIXED",
        fixed_rent or None,
        _safe_num(extract.get("revenue_share_pct")),
        min_guarantee or None,
        _safe_num(extract.get("rate_per_sqft")),
        _safe_num(extract.get("escalation_pct")),
        esc_freq, _add_months(lease_start, esc_freq),
        cam or None, hvac or None, marketing or None,
        _safe_num(extract.get("security_deposit")),
        _safe_num(extract.get("stamp_duty")),
        _safe_num(extract.get("registration_cost")),
        _safe_num(extract.get("capex_reimbursement")),
        _safe_int(extract.get("payment_due_day")),
        extract.get("renewal_option") or "MUTUAL",
        renewal_days, renewal_date,
        total_monthly or None,
        "spawar",
    ))

    run_dml(
        "UPDATE SI_EVENTS_HOL.LEASE_INTELLIGENCE.LEASE_STAGING SET STATUS = 'APPROVED' WHERE STAGING_ID = %s",
        (staging_id,),
    )
    ...
    try:
        ...
        if len(staging) > 0:
            ...
            for idx, row in pending.iterrows():
                ...
                with st.expander(f"📄 {fname} — Extracted {row['EXTRACTED_AT']}", expanded=(len(pending) <= 3)):
                    ...
                    with bcol2:
                        if st.button("Reject", key=f"reject_{sid}"):
                            try:
                                run_dml(
                                    "UPDATE SI_EVENTS_HOL.LEASE_INTELLIGENCE.LEASE_STAGING SET STATUS = 'REJECTED' WHERE STAGING_ID = %s",
                                    (sid,),
                                )
                                load_staging.clear()
                    ...
                    with bcol3:
                        if st.button("Re-extract", key=f"reextract_{sid}"):
                            try:
                                run_dml(f"""
                                    UPDATE SI_EVENTS_HOL.LEASE_INTELLIGENCE.LEASE_STAGING
                                    SET RAW_EXTRACT = (
                                        SELECT AI_EXTRACT(
                                            text => p.CONTENT,
                                            responseFormat => {{
                                        ...
                                        FROM SI_EVENTS_HOL.LEASE_INTELLIGENCE.PARSED_DOCS p
